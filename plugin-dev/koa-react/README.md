...
1. Install NodeMDA and this koa-react plugin globally on your system
...
2. Create a new project for your Koa/React application

    A. Develop a project using the NodeUML extension for VSCode

      Obtain a copy of [NodeUML](https://github.com/joelkoz/NodeUML) and follow the direction in the README file


    B. Use the CLI interface:
    ```
    ...
    ## A default StartUML project now exists in the 'model' directory
    ```


3. Using the NodeUML diagraming tool, StarUML, or any other UML modeler NodeMDA has a reader for, create or modify the *model* of your 
software architecture using the modeling convensions listed below. A UML Profile exists with pre-defined Stereotypes to quickly model database entities and services, as well as security requirements for access to both. 

4. The model is processed using NodeMDA to generate source code and other project artifacts. In general, NodeMDA generates
all of the boilerplate code and stubs for things such as your services and persistence entities.
    ...
    B. If using NodeUML:
    Right click on the "Model" and select "Generate code"

5. Where necessary, code stubs are filled in by hand to supply the business logic or add other functionality.

6. The resulting code is tested with manual testing.  See the README file created in your project directory after
